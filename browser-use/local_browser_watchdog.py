...
class LocalBrowserWatchdog(BaseWatchdog):
	...
	async def _launch_browser(self, max_retries: int = 3) -> tuple[psutil.Process, str]:
		...
		Uses Playwright for Google Chrome to ensure CDP compatibility.
		...
		for attempt in range(max_retries):
			try:
				# Check if this is Google Chrome (which needs Playwright launch for CDP compatibility)
				is_google_chrome = (
					profile.executable_path and 
					'google chrome' in profile.executable_path.lower() and
					'chromium' not in profile.executable_path.lower()
				)

				if is_google_chrome:
					# Use Playwright for Google Chrome to ensure CDP compatibility
					...
					# Use existing subprocess approach for other browsers
					return await self._launch_browser_via_subprocess(profile, attempt)
	...
	async def _launch_google_chrome_via_playwright(self, profile: BrowserProfile) -> tuple[psutil.Process, str]:
		...
		# Create a context to ensure CDP is available
	...
	async def _launch_browser_via_subprocess(self, profile: BrowserProfile, attempt: int) -> tuple[psutil.Process, str]:
		"""Launch browser using subprocess (original method for non-Chrome browsers)."""
		# Get launch args from profile
		launch_args = profile.get_args()
