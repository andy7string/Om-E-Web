...
class DefaultActionWatchdog(BaseWatchdog):
	...
	async def on_TypeTextEvent(self, event: TypeTextEvent) -> None:
		...
		try:
			...
			# Always attempt to type into the specific element first
			try:
	...
	async def _input_text_element_node_impl(self, element_node, text: str, clear_existing: bool = True):
		...
		try:
			...
			# Clear existing text if requested (use setRangeText when possible for real input events)
			if clear_existing:
				await cdp_session.cdp_client.send.Runtime.callFunctionOn(
					params={
			...
			# Avoid synthesizing extra characters to preserve exact value
