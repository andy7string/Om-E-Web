...
class TestSequentialAgentsSimple:
	...
	async def test_sequential_agents_share_browser_session_simple(self, httpserver):
		...
		initial_pid = (
			browser_session.local_browser_watchdog._subprocess.pid
			if browser_session.local_browser_watchdog and browser_session.local_browser_watchdog._subprocess
			else None
		)
